...
# 將 skills/topic-tracker/scripts 加入 Python 搜尋路徑，實現模組化代碼複用 (Single Source of Truth)
CURRENT_DIR = os.path.dirname(os.path.abspath(__file__))
SKILL_SCRIPTS_DIR = os.path.abspath(os.path.join(CURRENT_DIR, "..", "skills", "topic-tracker", "scripts"))
EXAMPLE_SCRIPTS_DIR = os.path.abspath(os.path.join(CURRENT_DIR, "..", "skill_example", "scripts"))

for s_dir in [SKILL_SCRIPTS_DIR, EXAMPLE_SCRIPTS_DIR]:
    if os.path.exists(s_dir) and s_dir not in sys.path:
...
def update_topic_tracking_time(topic_id: str, json_path: str = DEFAULT_JSON_PATH) -> str:
    ...
    try:
        if manage_topics:
            ...
            return f"SUCCESS: 已呼叫 manage_topics 成功更新議題 [{topic_id}] 之 last_tracked_at 時間。"
        else:
            # Fallback 獨立邏輯
            with open(json_path, 'r', encoding='utf-8') as f:
                data = json.load(f)
            ...
            for t in data.get("topics", []):
                if t.get("topic_id") == topic_id or t.get("topic_name") == topic_id:
